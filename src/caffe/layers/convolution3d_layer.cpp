...
void Convolution3DLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  kernel_size_ = this->layer_param_.convolution_3d_param().kernel_size();
  kernel_depth_ = this->layer_param_.convolution_3d_param().kernel_depth();
  stride_ = this->layer_param_.convolution_3d_param().stride();
  temporal_stride_ = this->layer_param_.convolution_3d_param().temporal_stride();
  pad_ = this->layer_param_.convolution_3d_param().pad();
  ...
  bias_term_ = this->layer_param_.convolution_3d_param().bias_term();
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.convolution_3d_param().weight_filler()));
    weight_filler->Fill(this->blobs_[0].get());
    // If necessary, initialize and fill the bias term
    if (bias_term_) {
      this->blobs_[1].reset(new Blob<Dtype>(1, 1, 1, 1, num_output_));
      shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
          this->layer_param_.convolution_3d_param().bias_filler()));
